...
def main(assembly_files, num_cycles):
    ...
    for assembly_file in assembly_files:
        ...
        if not os.path.exists("../../Desktop/快速临时处理文件夹/p2.2/my_tests/circ_files/reference_output"):
            os.makedirs("../../Desktop/快速临时处理文件夹/p2.2/my_tests/circ_files/reference_output")
        if not os.path.exists("../../Desktop/快速临时处理文件夹/p2.2/my_tests/input"):
            os.makedirs(r"../../Desktop/快速临时处理文件夹/p2.2/my_tests/input")

        # Creates ref file
        if num_cycles == -1:
            os.system("java -jar venus-jvm-latest.jar -t -tf trace_format -ti -tw -ts -r 32 " + assembly_file + " > " + ref_output)
        else:
            os.system("java -jar venus-jvm-latest.jar -t -tf trace_format -ti -tw -ts -r -tn " + str(num_cycles + 1) + " " + assembly_file + " > " + ref_output)
        os.system("del trace_format")

        # Creates hex file
        os.system("java -jar venus-jvm-latest.jar -d " + assembly_file + " > " + hex_file)
        os.system("copy " + assembly_file + " .\\my_tests\\input\\")
        ...
        os.system("move " + prefix + ".circ my_tests/circ_files/")
