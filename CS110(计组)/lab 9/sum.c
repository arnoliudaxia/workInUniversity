...
int sum_vectorized( int n, int *a )
{
    ...
    for( int i = 0; i < n/4*4; i += 4 )
    {
        sum=_mm_add_epi32( sum, _mm_loadu_si128(a+i) );
    }
    int result=0;
    //  _mm_storeu_si128(&result,sum);
    result+=sum[0]>>32;
    result+=sum[1]>>32;
    result+=sum[0]&0xffffffff;
    result+=sum[1]&0xffffffff;
    for( int i = n/4*4; i < n; i++ )   
    {
        result += a[i];
    }
...
{ 
    ...
    for( int i = 0; i < n/16*16; i += 16 )
    {
        ...
        _mm_add_epi32(_mm_loadu_si128(a+i+12),
        _mm_add_epi32(_mm_loadu_si128(a+i+8),
        _mm_add_epi32(_mm_loadu_si128(a+i),_mm_loadu_si128(a+i+4)))));
    }
